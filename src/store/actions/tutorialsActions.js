...
/**
 *
 * @param {number} limit the number of tags to consider
...
export const getTutorialsByTopTags =
  (limit = 10, userFilterHandle = undefined) =>
  async (firebase, firestore) => {
    let tutorialCollectionRef = firestore.collection("tutorials");

    // Query tutorials created by a specific user
    if (userFilterHandle) {
      tutorialCollectionRef = tutorialCollectionRef.where(
        "created_by",
        "==",
        userFilterHandle
      );
    ...
    const tutorialSnapshot = await tutorialCollectionRef
