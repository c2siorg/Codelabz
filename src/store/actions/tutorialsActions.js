...
export const uploadTutorialIcon = (icon,tutorial_id) => async (firebase) => {
  if(icon){
    try {
      const base64String = await convertToBase64(icon);
      const tutorialRef = firebase.firestore().collection('tutorials').doc(tutorial_id);
      await tutorialRef.update({
        icon: base64String,
      });
      return base64String

      console.log('Image uploaded and stored in Firestore:', base64String);
    } catch (error) {
